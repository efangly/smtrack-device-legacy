...
export class TemplogService {
  constructor(private readonly prisma: PrismaService, private readonly redis: RedisService, private readonly influxdb: InfluxdbService) {}
  async create(templogDto: CreateTemplogDto) {
    ...
    if (!tempLog.isAlert) {
      await this.influxdb.writeData(
        "templog", 
        `temp=${tempLog.tempValue},realTemp=${tempLog.realValue},door=${tempLog.door},plug=${tempLog.plugin}`, 
        { sn: tempLog.mcuId, ward: tempLog.device.ward, hospital: tempLog.device.hospital }
      );
    } else {
      await this.influxdb.writeData(
        "alert", 
